 ...
 * @author Marcio
 */
public class Avaliacao2 implements TimerListener{

    private Timer timer;
    private static int numberHosts = 10;
    ...
    public static void main(String[] args) throws Exception{
        Avaliacao2 test = new Avaliacao2();
            ...
            if(CloudSim.clock() >= cicle && !isACicle[j] && frontEnd.getLocalDatacenter().isRun()){
                ...
                while(!CloudSim.isPaused()){
                    try {
                        ...
                        Logger.getLogger(Avaliacao2.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
                int pValue = numberVms;
                numberVms += Avaliacao2.request(j);
                List<Vm> vmList = createVms(frontEnd.getId(), pValue);
                frontEnd.submitVmList(vmList);
                pValue = numberCloudlets;
                numberCloudlets += Avaliacao2.request(j);
                List<Cloudlet> cloudletList = createCloudletList(frontEnd.getId(), pValue);
                frontEnd.submitCloudletList(cloudletList);
                Log.printLine("Adding " + Avaliacao2.request(j) + " new Vms...");
                try {
