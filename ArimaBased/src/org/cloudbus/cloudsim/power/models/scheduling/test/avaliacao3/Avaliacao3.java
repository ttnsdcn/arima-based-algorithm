...
import org.cloudbus.cloudsim.power.models.scheduling.test.*;
 ...
 * @author Marcio
 */
public class Avaliacao3 implements TimerListener{
    ...
    public static void main(String[] args) throws Exception{
        Avaliacao3 test = new Avaliacao3();
            ...
            if(CloudSim.clock() >= cicle && !isACicle[j] && frontEnd.getLocalDatacenter().isRun()){
                ...
                while(!CloudSim.isPaused()){
                    try {
                        ...
                        Logger.getLogger(Avaliacao3.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
                int pValue = numberVms;
                numberVms += Avaliacao3.request(j);
                List<Vm> vmList = createVms(frontEnd.getId(), pValue);
                frontEnd.submitVmList(vmList);
                pValue = numberCloudlets;
                numberCloudlets += Avaliacao3.request(j);
                List<Cloudlet> cloudletList = createCloudletList(frontEnd.getId(), pValue);
                frontEnd.submitCloudletList(cloudletList);
                Log.printLine("Adding " + Avaliacao3.request(j) + " new Vms...");
                try {
