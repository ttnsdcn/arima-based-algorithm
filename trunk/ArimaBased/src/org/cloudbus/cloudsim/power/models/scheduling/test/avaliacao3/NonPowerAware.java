package org.cloudbus.cloudsim.power.models.scheduling.test.avaliacao3;
...
import org.cloudbus.cloudsim.power.models.scheduling.test.avaliacao2.*;
 ...
 */
public class NonPowerAware implements TimerListener{
	...
	public static void main(String[] args) {
            ...
            NonPowerAware test = new NonPowerAware();
		...
		try {
			...
			vmList = createVms(brokerId, 0);

			// submit vm list to the broker
			broker.submitVmList(vmList);

			// Fifth step: Create one cloudlet
			cloudletList = createCloudletList(brokerId, 0);
    ...
    public void update() {
        ...
        while(true){
            if(CloudSim.clock() >= cicle && !isACicle[j]){
                    CloudSim.pauseSimulation();
                    while(!CloudSim.isPaused()){
                        try {
                            Thread.sleep(100);
                        } catch (InterruptedException ex) {
                            Logger.getLogger(DVFS.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }


                        int pValue = (int) vmsNumber;
                        vmsNumber += NonPowerAware.request(j);
                        List<Vm> vmList = createVms(broker.getId(), pValue);
                        broker.submitVmList(vmList);
                        int pValue2 = (int) cloudletsNumber;
                        cloudletsNumber += NonPowerAware.request(j);
                        List<Cloudlet> cloudletList = createCloudletList(broker.getId(), pValue2);
                        broker.submitCloudletList(cloudletList);
                        Log.printLine("Adding " + NonPowerAware.request(j) + " new Vms...");
                        for(int i = 0; i < vmList.size(); i++)
                            broker.sendCloudletToEntity(cloudletList.get(i), datacenter, vmList.get(i), datacenter.getVmAllocationPolicy().allocateHostForVm(vmList.get(i)));
